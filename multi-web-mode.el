...
'open/'close to get the open/close regexp respectively."
  ...
  ;;   ('open (cadr tag))
  ;;   ('close (caddr tag))))
  (cond ((equal attribute 'mode) (car tag))
        ((equal attribute 'open) (cadr tag))
        ((equal attribute 'close) (car (cddr tag)))))
  ...
  (let ((index 0)
    ...
    (save-excursion
      ...
      (while (and (< index (length mweb-tags))
        ...
        (cond ((equal type 'nil) (setq tag-regexp (concat open-tag "\\|" close-tag)))
              ((equal type 'open) (setq tag-regexp open-tag))
              ((equal type 'close) (setq tag-regexp close-tag)))
